...
class BinaryClassification:
    ...
    def plot_confusion_matrix(self, threshold=None, normalize=False, title='Confusion matrix', cmap=plt.cm.Reds,
                              colorbar=False, label_rotation=45):
        """
        ...
        plt.tight_layout()
        plt.ylabel('True Perovskite')
        plt.xlabel('Predicted Perovskite')

        return cm

    def plot_roc_curve(self, threshold=None, plot_threshold=True, linewidth=2, y_text_margin=0.05, x_text_margin=0.2,
                       c_roc_curve='blue', c_random_guess='red', c_thresh_lines='black', ls_roc_curve='-',
        ...
        c_thresh_lines : string, default='blue'
        ...
        plt.tight_layout()
        plt.legend(loc=loc_legend,fontsize=14)

        return fpr, tpr, thresh, roc_auc

    def plot_precision_recall_curve(self, threshold=None, plot_threshold=True, beta=1, linewidth=2,
                                    fscore_iso=[0.6, 0.8], iso_alpha=0.7, y_text_margin=0.03,
                                    x_text_margin=0.2, c_pr_curve='black', c_mean_prec='red', c_thresh_lines='blue',
        ...
        ls_thresh : string, default='densely dashed'
            Define the linestyle of threshold lines.
        ...
        title : string, default="Precision-Recall Curve"
            Set title of the figure.
        Returns
        -------
        ...
        # Plot PR curve
        l, = plt.plot(recall, prec, color=c_pr_curve, lw=1, linestyle=ls_pr_curve, marker=marker_pr_curve)
        lines.append(l)
        ...
        if plot_threshold:
            # Plot vertical and horizontal line
            plt.axhline(y=idy_thresh, color=c_thresh_lines, linestyle=ls_thresh, lw=1)
            plt.axvline(x=idx_thresh, color=c_thresh_lines, linestyle=ls_thresh, lw=1)
            # Plot text threshold
            if idx_thresh > 0.5 and idy_thresh > 0.5:
                plt.text(x=idx_thresh - 4*x_text_margin, y=idy_thresh - 2*y_text_margin,
                         s='Threshold : {:.2f}'.format(t),fontsize=14)
            elif idx_thresh <= 0.5 and idy_thresh <= 0.5:
                plt.text(x=idx_thresh + x_text_margin, y=idy_thresh + y_text_margin,
                         s='Threshold : {:.2f}'.format(t))
            elif idx_thresh <= 0.5 < idy_thresh:
                plt.text(x=idx_thresh + x_text_margin, y=idy_thresh - 2*y_text_margin,
                         s='Threshold : {:.2f}'.format(t))
            elif idx_thresh > 0.5 >= idy_thresh:
                plt.text(x=idx_thresh - x_text_margin, y=idy_thresh + y_text_margin,
                         s='Threshold : {:.2f}'.format(t))
        ...
        # Axis and legends
        plt.xlim([0.0, 1.0])
        plt.legend(lines, labels,loc=3,fontsize=14)
        plt.xlabel('Recall',fontsize=14)
        plt.ylabel('Precision',fontsize=14)
        plt.tight_layout()
        if plot_threshold:
            plt.title('{} (Threshold = {})'.format(title, round(t, 2)),fontsize=14)
        else:
            plt.title(title)
    ...
    def plot_threshold(self, threshold=None, beta=1, title=None,
        ...
        c_thresh_line : string, default='blue'
            Color for the threshold line.
        lw_thresh_line : float, default=2
